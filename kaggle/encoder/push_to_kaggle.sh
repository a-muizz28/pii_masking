...
cp "$PROJECT_ROOT/notebooks/03_encoder_training.ipynb" "$SCRIPT_DIR/03_encoder_training.ipynb"
cd "$SCRIPT_DIR"
kaggle kernels push
cd "$PROJECT_ROOT"
...
sleep 10
kaggle kernels status "$KAGGLE_USER/pii-masking-day-3-training" || echo "  (Status may take a minute to appear)"
...
echo "  bash kaggle/encoder/check_status.sh"
echo ""
echo "When complete, pull results with:"
echo "  bash kaggle/encoder/pull_results.sh"
